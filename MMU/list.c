...
void list_add_ascending_by_address(list_t *l, block_t *blk){
  ...
  if(l->head == NULL){
    ...
    prev = current = l->head;
    if(current->next == NULL) {  //if there is only one node in list
       if(blk->start <= current->blk->start) {  
          newNode->next = l->head;
          l->head = newNode;
       }
       else {   // placing after the current node
          current->next = newNode;
          newNode->next = NULL;
       }
    }
    else {  // when there are two or more nodes in the list
       if(blk->start <= current->blk->start) {  // put in front of the current node
          newNode->next = l->head;
          l->head = newNode;
       }
       else {
...
void list_add_ascending_by_blocksize(list_t *l, block_t *blk){
  ...
  if(l->head == NULL){
    ...
    if(current->next == NULL) {  //if there is only one node in the list
       ...
       else {   // put it after the current node
          current->next = newNode;
          newNode->next = NULL;
       }
    }
    else {  // when there are two or more nodes in list
      
       if(newblk_size >= curblk_size) {  // put in front of the current node
          newNode->next = l->head;
          l->head = newNode;
       }
       else {
          ...
          while(current != NULL && newblk_size <= curblk_size) {
               ...
               if(current != NULL)  // if it is the last one in the list
